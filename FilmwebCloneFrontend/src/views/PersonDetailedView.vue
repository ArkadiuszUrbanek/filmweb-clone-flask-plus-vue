    ...
    <div v-if="person" class="bg-white flex items-center justify-center h-full">
      <div class="h-auto w-1/2 p-5 rounded-lg bg-white border shadow-lg">
        <div class="flex h-auto w-full bg-white">
          <img
            class="flex-shrink-0 h-[300px] overflow-hidden object-cover rounded"
            v-bind:src="person.file_path"
          />
          <div class="w-full bg-white ml-5">
            ...
            <p>
              ...
              >{{
                `${getFormattedBirthDateString} ${getFormattedAgeCalculation}`
              }}
            </p>
            <p>
              ...
              >{{ person.nationality }}
              <flag
                :iso="translatePersonNationalityTypeToISO"
                :squared="false"
              />
            </p>
            <p>
              <span class="font-medium">Height: </span>{{ person.height }} [cm]
            </p>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async getPerson() {
      try {
        const response = await axios.get<PersonDetailedInfoType>(
          `/actor/${this.$props.personId}`,
        );

        this.person = {
          ...response.data,
          birth_date: new Date(
            response.data.birth_date.toString(), // + 'T00:00:00.000Z',
          ),
  ...
  computed: {
    getFormattedBirthDateString(): string {
      ...
      const day = this.person!.birth_date.getUTCDate();
      const monthName = months[this.person!.birth_date.getUTCMonth()];
      const year = this.person!.birth_date.getUTCFullYear();
    ...
    getFormattedAgeCalculation(): string {
      const calculateAge = () => {
        ...
        const birthDateObj = new Date(this.person!.birth_date);
