...
def handle_410_gone(e):
    return Response(response = e.description, status = HTTPStatus.GONE, content_type = 'text/plain')

@errors_blueprint.app_errorhandler(InternalServerError)
def handle_500_internal_server_error(e):
    return Response(response = e.description, status = HTTPStatus.INTERNAL_SERVER_ERROR, content_type = 'text/plain')